...
import { supabase, getActiveSchema, SUPABASE_SCHEMA } from '../supabase';
...
function ParentPortal({ parentData, setParentData }) {
   ...
   useEffect(() => {
      ...
      const unreadChan = supabase.channel(`parent_unread_${mahv}`)
         .on('postgres_changes', { event: '*', schema: getActiveSchema(), table: 'hv_messages', filter: `mahv=eq.${mahv}` }, () => {
            fetchUnreads();
         })
         .subscribe();

      const chatChan = supabase.channel(`parent_chat_${mahv}`)
         .on('postgres_changes', { event: 'INSERT', schema: getActiveSchema(), table: 'hv_messages', filter: `mahv=eq.${mahv}` }, (payload) => {
         ...
         .on('postgres_changes', { event: 'UPDATE', schema: getActiveSchema(), table: 'hv_messages', filter: `mahv=eq.${mahv}` }, (payload) => {
            setChatMessages(prev => prev.map(m => m.id === payload.new.id ? payload.new : m));
         })
         .subscribe();

      const noticeChan = supabase.channel(`parent_notices_${mahv}`)
         .on('postgres_changes', { event: '*', schema: getActiveSchema(), table: 'tbl_thongbao', filter: `mahv=eq.${mahv}` }, async (payload) => {
         ...
         .on('postgres_changes', { event: '*', schema: getActiveSchema(), table: 'tbl_hd', filter: `mahv=eq.${mahv}` }, async () => {
            await refreshLatestFeeData();
            await fetchUnreads();
         })
         .on('postgres_changes', { event: '*', schema: getActiveSchema(), table: 'class_announcements', filter: `malop=eq.${malop}` }, async (payload) => {
            await fetchUnreads();
            await fetchParentNotices();
         ...
         .on('postgres_changes', { event: '*', schema: getActiveSchema(), table: 'suckhoedinhky', filter: `mahv=eq.${mahv}` }, async (payload) => {
            await fetchUnreads();
            if (payload.eventType === 'INSERT') showNotification('Cập nhật sức khỏe', 'Bé vừa có thông tin sức khỏe mới');
         })
         .on('postgres_changes', { event: '*', schema: getActiveSchema(), table: 'tbl_hv', filter: `mahv=eq.${mahv}` }, async (payload) => {
            setParentData(prev => {
               if (!prev?.student) return prev;
               return { ...prev, student: { ...prev.student, ...payload.new } };
            });
         })
         .on('postgres_changes', { event: '*', schema: getActiveSchema(), table: 'dangkyngoaikhoa', filter: `mahv=eq.${mahv}` }, async (payload) => {
