   ...
   useEffect(() => {
      if (attTab === 'chat' && attChatSelectedStudent) {
         ...
         const threadChannel = supabase.channel(`att_thread_${attChatSelectedStudent.mahv}`)
            .on('postgres_changes', { event: 'INSERT', schema: getActiveSchema(), table: 'hv_messages', filter: `mahv=eq.${attChatSelectedStudent.mahv}` }, (payload) => {
            ...
            .on('postgres_changes', { event: 'UPDATE', schema: getActiveSchema(), table: 'hv_messages', filter: `mahv=eq.${attChatSelectedStudent.mahv}` }, (payload) => {
