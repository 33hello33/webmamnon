...
function FileDropZone({
  ...
  const forwardFiles = (rawFiles) => {
    ...
    const nextFiles = typeof filterFiles === 'function'
    ...
    >
      ...
      {resolvedFiles.length > 0 ? (
        <>
          ...
          {selectedHint ? (
            <span style={{ fontSize: '0.8rem', color: subTextColor, textAlign: 'center' }}>
              {selectedHint}
            </span>
          ) : null}
        </>
        ...
        <>
          {emptyTitle ? (
            <span style={{ fontWeight: 700, color: textColor, textAlign: 'center' }}>
              {emptyTitle}
            </span>
          ) : null}
          {emptySubtitle ? (
